...
function determine_sprite(equipment = "",action = ""){
	...
	var is_attacking = struct.state_machine.state.name == "attack" or struct.state_machine.state.name == "attack_windup"
...
function reset_timers(){
	if(not_null(time_sources)){
		var timer_count = array_length(time_sources)
		if(timer_count > 0){
			for(var i = 0; i < timer_count;i++){
				var current_timer = time_sources[i];
				if(time_source_get_state(current_timer) == time_source_state_active){
					time_source_reset(current_timer);
				}
			}
		}
	}
